...
class SegTracker():
    ...
    def seg(self, frame):
        ...
        # note that the merged mask may lost some objects due to the
        # overlapping
        ...
        self.object_idx = 1
        for id in obj_ids:
            if (np.sum(self.origin_merged_mask == id) < self.min_area or
                self.object_idx > self.max_obj_num):
                self.origin_merged_mask[self.origin_merged_mask == id] = 0
            else:
                self.origin_merged_mask[self.origin_merged_mask == id] = (
                    self.object_idx)
                self.object_idx += 1
    ...
    def add_reference(self, frame, mask, frame_step=0):
        ...
        self.tracker.add_reference_frame(frame, mask, self.curr_idx - 1,
                                         frame_step)
    ...
    def track(self, frame, update_memory=False):
        ...
        return pred_mask.squeeze(0).squeeze(0).detach().cpu().numpy().astype(
            np.uint8)
    ...
    def find_new_objs(self, track_mask, seg_mask):
        ...
        for idx in new_obj_ids:
            ...
            if (new_obj_area / obj_area < self.min_new_obj_iou or
                new_obj_area < self.min_area or
    ...
    def seg_acc_click(self, origin_frame: np.ndarray, coords: np.ndarray,
                      modes: np.ndarray, multimask=True):
        '''
    ...
    def add_mask(self, interactive_mask: np.ndarray):
        ...
        if self.origin_merged_mask is None:
            self.origin_merged_mask = np.zeros(interactive_mask.shape,
                                               dtype=np.uint8)
    ...
    def detect_and_seg(self, origin_frame: np.ndarray, grounding_caption,
                       box_threshold, text_threshold, box_size_threshold=1,
                       reset_image=False):
        '''
        ...
        annotated_frame, boxes = self.detector.run_grounding(
        ...
        for i in range(len(boxes)):
            ...
            if area > frame_area * box_size_threshold:
                continue
            interactive_mask = self.sam.segment_with_box(
                origin_frame, bbox, reset_image)[0]
            refined_merged_mask = self.add_mask(interactive_mask)
